  ...
  if (req.method === 'POST') {
    try {
      ...
      if (!title || !type || price === undefined) {
        return new Response(JSON.stringify({ message: '필수 정보가 누락되었습니다. (title, type, price)' }), { 
          status: 400, 
          headers: { 'Content-Type': 'application/json' } 
        });
      }
      ...
      if (!supabaseUrl || !supabaseKey) {
        return new Response(JSON.stringify({ message: '매물이 등록되었습니다. (로컬 모드)' }), { status: 201, headers: { 'Content-Type': 'application/json' } });
      }

      const supabase = createClient(supabaseUrl, supabaseKey);
      const { error } = await supabase
        .from('listings')
        .insert([{ title, type, price, description }]);

      if (error) throw error;

      return new Response(JSON.stringify({ message: '매물이 등록되었습니다.' }), { 
        status: 201, 
        headers: { 'Content-Type': 'application/json' } 
      });
